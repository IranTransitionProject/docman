...
The ``model_used`` field in the result indicates which backend ran:
...
Configuration (via backend_config in worker YAML):
    ...
    # Docling-specific (passed through when fallback triggers):
    device, num_threads, ocr_engine, do_ocr, do_table_structure,
    layout_batch_size, ocr_batch_size

See Also:
    configs/workers/doc_extractor_smart.yaml -- worker config
    docman.backends.markitdown_backend.MarkItDownBackend -- fast path
    docman.backends.docling_backend.DoclingBackend -- fallback path
"""
    ...
    def process_sync(self, payload: dict[str, Any], config: dict[str, Any]) -> dict[str, Any]:
        ...
        if suffix in force_docling:
            logger.info(
                ...
                extra={"file_ref": file_ref, "reason": f"extension {suffix} in force list"},
            )
        ...
        if len(text_preview.strip()) < min_text_length:
            ...
            return self._fallback_to_docling(
                payload, config, reason=f"text too short ({text_len} < {min_text_length})"
            )
    ...
    def _fallback_to_docling(
        ...
        *,
        ...
        Args:
            ...
            config: Backend config dict (Docling options are passed through).
            reason: Human-readable reason for fallback (logged).
